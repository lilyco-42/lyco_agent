...
# v2 教训: 英文 shell 占 80% 会冲淡中文硬件任务并把拒绝率打到 20% -> v3 纯中文 + 说法族扩充,
...
OUT3 = "/workspace/qwen3_router_v3"

print("=== CLI router v3 (pure zh, richer paraphrase families) ===", flush=True)
    ...
    for _ in range(700):
        ...
        if q not in seen:
            ...
            hw_train.append([{"role": "user", "content": q},
                             {"role": "assistant", "content": make_cmd(cap, kw)}])
for t in NOOP_TRAIN:
    for _ in range(40):      # 拒绝类要足够多, 否则拒绝率崩
        hw_train.append([{"role": "user", "content": t},
...
# ---- 训练 v3 (纯中文, repo 配方: lr 2e-5 / 4 epoch)
rows = [encode(HW_SYSTEM, m) for m in hw_train]
print(f"\ntrain rows={len(rows)}", flush=True)
model = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.bfloat16,
                                             attn_implementation="sdpa").cuda()
args = TrainingArguments(output_dir=OUT3, num_train_epochs=4, per_device_train_batch_size=16,
...
model.save_pretrained(OUT3)
tok.save_pretrained(OUT3)
print("V3_SAVED", flush=True)

for name, cases in (("heldA", held_a), ("heldB", held_b)):
    a, r, tc, tr = evaluate(model, cases)
    print(f"v3 {name}: cmd={a:.1%} ({int(a*tc)}/{tc})  reject={r:.1%} ({int(r*tr)}/{tr})", flush=True)
