...
MERGED, V6 = "/workspace/qwen3_router_merged", "/workspace/qwen3_router_v6"
...
def make_cmd(cap, kw):
    # 注意: 不能用 dict 字面量 —— 所有 f-string 会【立即求值】, 缺槽位即 KeyError
    # (这条仓库 cli_datagen.py 里明文写过, 我第一版仍踩了) 必须用 if 分支惰性求值
...
# ---------- ① 参照：合并模型（v5 已产出；本 run 复测以保证同分布可比） ----------
print("\n=== reference: merged (v3 ⊕ v4) ===", flush=True)
m4 = load(MERGED); report("merged(reference)", m4)
...
print("(merged ckpt 复用 v5 产出, 本 run 不再重新合并)", flush=True)

# ---------- ③ verifier-GRPO：从【合并模型】续训（报告 5.1.2 的标准流程：merge → reinit → RL） ----------
print("\n=== verifier-GRPO from MERGED ===", flush=True)
del m4
torch.cuda.empty_cache()
from trl import GRPOConfig, GRPOTrainer
from datasets import Dataset

init = MERGED if os.path.isdir(MERGED) else (V4 if os.path.isdir(V4) else MODEL_ID)
...
cfg = GRPOConfig(output_dir=V6, num_generations=6, per_device_train_batch_size=12,
...
model.save_pretrained(V6); tok.save_pretrained(V6)
print("V6_SAVED", flush=True)
report("v6(merge→verifier-GRPO)", model)
