//! CloudStudio Jupyter 内核执行器 (long-timeout 版): 连 channels ws 跑 python, 收集输出。
//! 与 cs_exec3.mjs 相同, 但超时改为 60 分钟, 用于大体积 pip install / 训练。
//! 用法: node cs_exec_long.mjs --file <path.py>
...
function execOnKernel(kid, code) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => { finish({ error: 'TIMEOUT', out: out.join('') }); }, 3600000);
