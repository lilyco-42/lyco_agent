...
> ## 🔴 2026-09-21 口径修正（用户明确指示，覆盖本文档原文需求）
>
> 用户原话：**「需求写的是『7 个固定工具』。我根本不需要这个，
> 我强调 brush shell 实现。泛用性和对未知 cli 未来 10 年发展依旧可用。」**
>
> **修正后的需求（现行 of record）**：
> > **以 `brush`（通用 shell）为唯一工具出口，把自然语言意图翻译成可执行的 CLI 命令。
> > 验收标准 = 泛用性：对训练中未见过的 CLI 也能给出可执行命令（靠 schema 注入而非重训），
> > 危险/越权请求由 T1 门拦截。全程零重训即可扩展到新 CLI。**
>
> - ❌ **废弃**：「7 个固定工具各 ≥90%」——封闭枚举，新工具出现即失效，撑不了 10 年。
> - ✅ **现行**：开放世界 + 注入协议。新 CLI = 加一段 schema 文本，**不改模型、不重训**。
> - 详细论证与实测数据：`docs/requirement-brush-generality-2026-09-21.md`
>
> ⚠️ **本文档下文（原文）保留为历史记录**，其中「7 个固定工具」相关表述**已失效**；
> 但 §二/§三/§四 中的技术选型（Hammer / xLAM / Unsloth QAT / 窄缝自研）**仍然有效**。
