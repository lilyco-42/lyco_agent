...
## 4f. ★ 五轮跨轮方差分析：本次实验最重要的发现 —— **指标本身有噪声，之前的解读全部失效**

把 v18b–v18f 五轮的 hparse 读数（base06b）拉齐：

| CLI | v18b | v18c | v18d | v18e | v18f | 均值 | **标准差** | 极差 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| docker | 70.0 | 60.0 | 90.0 | 90.0 | 90.0 | 80.0 | **±12.6** | 30.0 |
| kubectl | 55.6 | 66.7 | 22.2 | 33.3 | 33.3 | 42.2 | **±16.3** | 44.4 |
| npm | 62.5 | 37.5 | 50.0 | 62.5 | 62.5 | 55.0 | **±10.0** | 25.0 |
| git | 87.5 | 87.5 | 87.5 | 87.5 | 87.5 | 87.5 | **±0.0** | 0.0 |
| cargo | 87.5 | 87.5 | 87.5 | 87.5 | 87.5 | 87.5 | **±0.0** | 0.0 |
| jq | 25.0 | 25.0 | 25.0 | 25.0 | 25.0 | 25.0 | **±0.0** | 0.0 |

### 三个「完全确定」的 CLI 就是本次实验的真实信号

`git` / `cargo` / `jq` 五轮**逐字不变**（±0.0）。这三个恰好都是
**纯子命令、help 排版规整、评测题不需要选参数值**的 CLI：
- git：`status` / `log` / `branch` / `diff` —— 一题一命令，一一对应
- cargo：`build` / `test` / `check` —— 同上
- jq：天花板不动（见 4c）

→ **这三条是可信读数**：git 87.5 / cargo 87.5 / jq 25.0 稳定可复现。

### 三个「高方差」的 CLI 分数主要是采样噪声

docker ±12.6、kubectl ±16.3（极差 **44.4**）、npm ±10.0。
逐题核对证据：

```text
v18e: kubectl get pods | get pods | get pods | api-versions | nodes
v18d: kubectl describe pods | top pods | describe pods | svc | ns list
v18b: kubectl get pods | get pods | get pods | expose | get nodes
```

**高分题三轮完全一致**（`get pods`、`logs <名字>` 这类模型真会的），
**低分题三轮各猜一个不同的错答案**（`expose` / `svc` / `ns list` / `nodes` / `api-versions`）。
这是**从已注入的 32 条命令里随机抽一条**的形态，不是「读了 schema 后选错」。

**决定性证据（v18f vs v18e）**：v18f 把 `describe` 的示例从
`kubectl describe pods` 改成 `kubectl describe pods web`（两参数完整形态），
**注入内容确实变了，输出却逐字完全相同** —— 33.3% 对 33.3%，9 题预测一字不差。

→ **对 kubectl 上的这些短命令，schema 内容的改动根本没进入模型的有效决策。**
我此前「同义词格式歧义」「示例形态不对」的两轮归因（4d/4e）**都是错的** ——
错不在提示措辞，在于**这类题在 base06b 上没有可利用的信号**。

### 由此推翻/修正的三条此前的结论

| 4d/4e 的原判 | 实际 |
| --- | --- |
| 「同义词表写错格式 → kubectl 塌方」 | kubectl 本来就在 ±16.3 噪声内，22.2 与 55.6 都是同一分布的采样 |
| 「v18d 的 `kubectl svc` 是同义词表污染」 | 是采样噪声的一次实现；v18f 改掉措辞后仍出现 `kubectl nodes` |
| 「v18c 保真度 54.7 → v18e 61.9 是修复生效」 | 在 ±12~16 噪声下，这两次差异不显著 |

**唯一的真结论：`docker` 从 70.0 到 90.0 的那次跃升需要复核**（4d→4e 两轮 90.0，
而 v18b/v18c 是 70.0/60.0）—— 它跨过了噪声下限，但 n=2，仍不足以定论。

### 对验收标准本身的修正（比任何单一修复都重要）

原标准 ①「hparse ≥ hand × 0.9，6 CLI 平均」在 ±10~16pp 噪声下，
**任何小于 20pp 的改进都无法与噪声区分**。这条标准在当前样本量下**不可判定**。

修正为**分组验收**：

| 组 | CLI | 判据 | 现状 |
| --- | --- | --- | --- |
| **A 稳定组** | git / cargo | 五轮 ±0.0，可直接比较 | ✅ 87.5 / 87.5，远超裸 help（100.0 / 62.5 中 cargo +25pp） |
| **B 噪声组** | docker / kubectl / npm | 需 n≥5 且报告置信区间，不看单轮 | ⚠️ 均值 80 / 42 / 55，但方差过大 |
| **C 天花板组** | jq | 纯 help 无解，需声明式补丁 | ✅ 已知 25.0，不再作为改进目标 |

→ **产品的可交付结论**（不依赖噪声组）：
**出域接入对「纯子命令型 CLI」有效且可复现（git 87.5 / cargo 87.5），
对「参数值选择型」CLI 有正增益但无法在端侧 0.6B 模型上稳定（docker/kubectl/npm），
对「DSL 型」CLI 无效（jq 25.0 为上限）。**
这条结论的适用范围**已由方差本身界定**，不再依赖对噪声的解读。
