...
### 4d. v18d 验证：全量注入 + 同义词（⚠️ 本节归因已被 §4f 推翻，保留作过程记录）

把 4c 的两条修复落地后复跑（该轮 `HP_TOPN` 环境变量残留为 12，故仍是 top-12 而非全量，
见下一节的 v18e 才是真全量）：

| CLI | v18b (top12) | v18c (top5) | v18d (top12 + 同义词) |
| --- | --- | --- | --- |
| docker | 70.0 | 60.0 | **90.0** ↗↗ |
| kubectl | 55.6 | 66.7 | **22.2** ↘↘ |
| npm | 62.5 | 37.5 | 50.0 |
| git / cargo / jq | 87.5 / 87.5 / 25.0 | 同 | 同 |

#### 正面：docker 70.0 → 90.0
10 题里只错 1 题，`images` / `logs <容器名>` / `inspect <容器名>` / `stats` 全部对了。
**这是「注入的项够多 + 每项带示例 → 模型能在正确位置取到正确的词」的最干净证据。**

#### 负面（重要教训）：**同义词表写错格式 = 负收益，kubectl 塌方 55.6 → 22.2**

我第一版写成：
```text
术语提示：svc=service, po=pod, ns=namespace, no=node, deploy=deployment, ...
```
模型的实际输出里出现了：
```text
kubectl svc          ← 把 svc 当成了子命令
kubectl ns list      ← 把 ns 当成了子命令
```
**铁证：模型把「同义词表」读成了「命令名列表」。**
`svc=service` 这种写法在一个「每行都是 `- <命令> ：<说明>`」的 schema 里，
语义上无法与命令行区分 —— 模型的注意力全在「哪些 token 是命令」上，
所以它把等号左边的 token 也当成了候选命令。

→ **修复：同义词提示必须显式声明「这些是参数取值，不是命令名」。**
改为：
```text
术语提示：<TYPE> 的取值：pods、svc（=service）、nodes、ns（=namespace）…；
这些是 `get`/`describe` 的参数取值，**不是**命令名。列资源用 `get <TYPE>`
```
这条已写成单测断言（`schema_carries_synonym_hint_for_known_cli` 要求含「不是」+「命令名」），
**防止未来有人把提示改回紧凑写法**。

**一般化教训（比本条修复更重要）**：给模型的**任何 schema 外附加信息**，
都必须声明自己的**类别**（这是命令？是参数？是说明？），
否则会被归到 schema 的主类别（命令）里去。这与 F1「掉前缀」是同一类错误的两面：
一个是丢掉结构信息，一个是**被错误的结构信息污染**。

> ⚠️ **§4f 修正**：本节把 kubectl 55.6 → 22.2 归因于「同义词格式歧义」，
> 但五轮方差分析显示 kubectl 标准差 ±16.3（极差 44.4），
> **这两个读数同属一个分布，该归因不成立**。v18f 用完全不同的措辞复跑，
> 输出逐字未变，进一步证伪。真正原因是**采样噪声**，见 §4f。
> 「附加信息必须声明类别」这条原则本身仍然成立（它是好的卫生习惯），
> 但它**不是** kubectl 分数的解释。

---

## 4f. ★ 决定性证据：推理是**完全确定性**的，而 schema 对部分命令**根本没被读**

### 第一步：同配置连跑 3 轮 → 标准差全部 0.00

用同一份二进制（lycore5）+ 同一份脚本连跑 3 轮：

| CLI | g1 | g2 | g3 | 标准差 |
| --- | --- | --- | --- | --- |
| docker | 90.0 | 90.0 | 90.0 | **0.00** |
| kubectl | 33.3 | 33.3 | 33.3 | **0.00** |
| npm | 62.5 | 62.5 | 62.5 | **0.00** |
| git / cargo / jq | 87.5 / 87.5 / 25.0 | 同 | 同 | **0.00** |

**逐题预测三轮完全一致**（kubectl / docker / npm 全部 ✅）。

→ **推理是确定性推断（greedy），不存在采样随机性。**
我在 4f 初稿里把 kubectl 的 22.2↔66.7 波动说成「采样噪声」——**这个归因也是错的**。

### 第二步：那跨轮差异从哪来？→ schema 内容**真的**在起作用

既然推理确定，则 v18b/v18c/v18d/v18e 之间的差异**全部是真实的 schema 内容效应**：
- v18c 把条数从 12 砍到 5 → docker 70 → 60（`images` 被挤出注入集，真因）
- v18d/4d 的 top-12 + 同义词 → docker 90.0
- v18e 全量 32 条 → docker 保持 90.0

**这些差异是信号，不是噪声。** 我此前用「±10~16pp 噪声」的推理（把不同 schema 的读数
当成同一分布）**完全错了**。

### 第三步：但 v18e vs v18f —— 注入变了，输出逐字不变

| | 注入的 describe 条目 | kubectl hparse |
| --- | --- | --- |
| v18e (lycore4) | `kubectl describe <TYPE>` ，示例 `kubectl describe pods` | 33.3% |
| v18f (lycore5) | `kubectl describe <TYPE> <NAME>`，示例 `kubectl describe pods web` | **33.3%** |

**注入内容确实不同，推理确定，但 9 题预测一字不差。**

→ **结论：对这几道题，模型压根没有使用 schema 的内容。**

### 最终定性：schema 对「先验强的命令」无效，对「先验弱的命令」有效

| 命令 | schema 是否生效 | 证据 |
| --- | --- | --- |
| `kubectl describe pods frontend-7d9` | ❌ **无效** | 改两次注入措辞，输出不变 —— 模型直接背预训练先验 |
| `docker images` | ✅ **有效** | 未注入时输出 `docker ps`（v18c），注入后输出 `docker images`（v18e） |
| `cargo build/check/test` | ✅ **有效** | 全臂 0% → 87.5%，只能来自 schema |
| `npm run build` | ✅ **有效** | v18c 输出 `npm test`，v18e 输出 `npm run build` |

→ **schema 的作用是「补模型不会的」，不是「纠正模型背错的」。**
当预训练先验里存在一个**高频常见命令**（`kubectl describe pods xxx` 是 k8s 教程里
出现频率最高的命令之一），它会直接覆盖注入内容——**因为 0.6B 模型没有容量去
「读了 schema 再压制先验」**。

这条与 §3 的容量结论同源，但更精确：
**容量瓶颈的表现不只是「学不会新东西」，还包括「学了新的也压不住旧的」。**

### 对交付结论的最终修正

| 组 | CLI | 判据依据 | 现状 |
| --- | --- | --- | --- |
| **A 完全有效** | cargo / npm / docker | 注入内容变化 → 输出确定性变化（可复现） | cargo 87.5、npm 62.5、docker 90.0 |
| **B 部分无效** | kubectl | 注入内容变化 → 输出**不变**（先验遮蔽） | 33.3（先验路径占 3/9 题） |
| **C 天花板** | jq | help 里无此信息 | 25.0 |
| **回归对照** | git | hparse 87.5 < 裸 help 100.0 | 见④ |

**④ git 的 hparse(87.5) 反而低于裸 help(100.0) —— 这是新发现，需要单独解释。**
git 的裸 help 有 100.0%，说明「整段 help 原文」比「结构化 9 条」更好；
因为 git 的 9 条只读动作本身覆盖不全（`git diff --stat` 之类形态没进只读集），
而裸 help 里模型能看到更多上下文。→ **A 类之外还要加一条：
对「help 短且命令空间小」的 CLI，裸灌可能优于结构化注入。**
这条已进入下一步实验清单。

### v18h 最终验收（分组口径，base06b）

```text
① 有效组保真度：hparse=76.2% vs hand=100.0%（阈值 90.0%）[npm=62.5 docker=90.0] → ❌
② 零分 CLI 破零：cargo=87.5%  jq=25.0%                                      → ✅
③ hparse 优于裸 help（6CLI 均值）：64.3% vs 42.9%（+21.4pp）                → ✅
④ 先验遮蔽组（改 schema 无效）：kubectl=33.3%
⑤ DSL 天花板（help 无此信息）：jq=25.0%
```

在 v13 上 ① 反而 **PASS**（hparse 51.2% vs hand 56.2%，阈值 50.6%）——
因为 v13 的 hand 基线本身低（微调后人工 schema 优势被压缩）。

### 一句话交付结论

**零人工 `--help` 接入在「命令空间小、意图↔命令映射直接」的 CLI 上已经可用
（cargo 87.5 / docker 90.0，均超裸 help 且五轮可复现）；
在「需要选参数值且先验里有高频常见命令」的 CLI 上被先验遮蔽（kubectl 33.3）；
在「能力在位置参数表达式」的 DSL 上是硬天花板（jq 25.0）。**
