    ...
    for y in 1..h - 1 {
        for x in 1..w - 1 {
            ...
            if mag > 24.0 {
                ...
                let bin = (((ang + std::f32::consts::PI) / (2.0 * std::f32::consts::PI)) * 16.0)
...
pub fn identify(
    ffmpeg: &str,
    image: &Path,
) -> anyhow::Result<(String, f64, Vec<ExpertScore>, Vec<String>)> {
    let feat = extract_features(ffmpeg, image)?;

    // ---- CNN 通道 (v2/v3 训练版, 4 类) ----
    if let Some(wpath) = crate::vnn_cnn::CnnClassifier::resolve() {
        if let Ok((probs, votes)) = crate::vnn_cnn::CnnClassifier::load(&wpath).and_then(|clf| {
        ...
        // CNN 不可用 (加载/推理 Err) → 跳到末尾规则回退 (诚实降级)
    }
...
fn rule_identify(feat: &Features) -> anyhow::Result<(String, f64, Vec<ExpertScore>, Vec<String>)> {
    ...
    let experts: Vec<ExpertScore> = activated
        ...
        .collect();
    let trained_best = experts.iter().filter(|e| !e.needs_training).max_by(|a, b| {
        (a.conf * a.activation)
